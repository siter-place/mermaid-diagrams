...
- [x] Implement `UpgradeRunner` targeting DB version `1.3.1` with `{$prefix}mdm_usage` and `{$prefix}mdm_usage_dirty` tables
...
| Database | Usage tables | `mdm_usage` and `mdm_usage_dirty` created | Version 1.3.1 schema |
