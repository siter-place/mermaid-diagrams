...
- [x] Execute post-phase documentation and automated versioning closeout rule (`.cursor/rules/post-phase-documentation.mdc`) updating `.env`, version synchronization to `1.4.1`, decision log, and changelog.
...
### Modified Files:
- `mermaid-diagrams.php` (added `MDM_MERMAID_VERSION` constant, updated version header to `1.4.1`)
- `src/Bootstrap/Plugin.php` (registered `RestServiceProvider`)
- `package.json` & `package-lock.json` (updated version to `1.4.1` and `test:rest` script)
- `CHANGELOG.md` (promoted Unreleased items under `[1.4.1] - 2026-08-01`)
- `docs/decision-log.md` (added `REL-1.4.1` decision log entry)
