...
export async function deleteDiagram(
  ...
  try {
    const path = `${REST_NAMESPACE}/diagrams/${id}?force=${force ? 'true' : 'false'}`;
    return await apiFetch<{ deleted: boolean; id: number; force: boolean }>({
      path,
      method: 'DELETE',
    });
  } catch (error) {
 ...
 */
export async function getDiagramUsage(id: number): Promise<DiagramUsageResponse> {
  try {
